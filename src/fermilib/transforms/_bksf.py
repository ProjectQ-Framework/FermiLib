...
def bravyi_kitaev_fast(operator):
    """ Find the bravyi_kitaev_fast pauli-representation of
    InteractionOperator. bravyi_kitaev_fast pauli-representation of general
    FermionOperator is not possible in bravyi_kitaev_fast. So,the
    InteractionOperator given as input must be Hermitian.
    Returns:
...
def bravyi_kitaev_fast_interaction_op(iop, n_qubits=None):
    """Output InteractionOperator as QubitOperator class under
    bravyi_kitaev_fast transform.
        ...
        for q in range(n_qubits):
            ...
            if coefficient and p >= q:
                qubit_operator += coefficient * \
                                 bravyi_kitaev_fast_one_body(edge_matrix, p, q)
            ...
            for r in range(n_qubits):
                for s in range(n_qubits):
                    ...
                    transformed_term = bravyi_kitaev_fast_two_body(edge_matrix,
                                                                   p, q, r, s)
...
def bravyi_kitaev_fast_edge_operator_b(edge_matrix, i):
...
def bravyi_kitaev_fast_edge_operator_aij(edge_matrix, i, j):
...
def bravyi_kitaev_fast_one_body(edge_matrix, p, q):
    ...
    if p != q:
        a, b = sorted([p, q])
        B_a = bravyi_kitaev_fast_edge_operator_b(edge_matrix, a)
        B_b = bravyi_kitaev_fast_edge_operator_b(edge_matrix, b)
        A_ab = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, a, b)
        qubit_operator += (-1j/2)*(A_ab*B_b+B_a*A_ab)

    # Handle diagonal terms.
    else:
        B_p = bravyi_kitaev_fast_edge_operator_b(edge_matrix, p)
        qubit_operator += (QubitOperator((), 1)-B_p)/2

    return qubit_operator


def bravyi_kitaev_fast_two_body(edge_matrix, p, q, r, s):
    ...
    # Handle case of four unique indices.
        ...
        B_p = bravyi_kitaev_fast_edge_operator_b(edge_matrix, p)
        B_q = bravyi_kitaev_fast_edge_operator_b(edge_matrix, q)
        B_r = bravyi_kitaev_fast_edge_operator_b(edge_matrix, r)
        B_s = bravyi_kitaev_fast_edge_operator_b(edge_matrix, s)
        A_pq = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, p, q)
        A_rs = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, r, s)
    ...
    # Handle case of three unique indices.
        ...
        if p == r:
            ...
            B_q = bravyi_kitaev_fast_edge_operator_b(edge_matrix, q)
            B_s = bravyi_kitaev_fast_edge_operator_b(edge_matrix, s)
            A_qs = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, q, s)
            ...
            B_q = bravyi_kitaev_fast_edge_operator_b(edge_matrix, q)
            B_r = bravyi_kitaev_fast_edge_operator_b(edge_matrix, r)
            A_qr = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, q, r)
            ...
            B_q = bravyi_kitaev_fast_edge_operator_b(edge_matrix, q)
            B_s = bravyi_kitaev_fast_edge_operator_b(edge_matrix, s)
            A_ps = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, p, s)
            ...
            B_q = bravyi_kitaev_fast_edge_operator_b(edge_matrix, q)
            B_r = bravyi_kitaev_fast_edge_operator_b(edge_matrix, r)
            A_pr = bravyi_kitaev_fast_edge_operator_aij(edge_matrix, p, r)
...
def bravyi_kitaev_fast_vacuum(edge_matrix):
    ...
    for stab in stabs:
        ...
        for i in range(numpy.size(stab)):
            if i == (numpy.size(stab) - 1):
                A = (1j)*A * \
                    bravyi_kitaev_fast_edge_operator_aij(edge_matrix,
                                                         stab[i], stab[0])
        ...
        op = op*(QubitOperator(()) + A)/numpy.sqrt(2)

    return op


def bravyi_kitaev_fast_number_operator(iop, qubit_number=None):
    """Find the qubit operator for the number operator in bravyi_kitaev_fast
       representation"""
    n_qubit = iop.n_qubits
    op = QubitOperator()
    edge_matrix = bravyi_kitaev_fast_edge_matrix(iop)
    if qubit_number is None:
        for i in range(n_qubit):
            op += (QubitOperator(()) -
                   bravyi_kitaev_fast_edge_operator_b(edge_matrix, i))/2

    else:
        op += (QubitOperator(()) -
               bravyi_kitaev_fast_edge_operator_b(edge_matrix, qubit_number))/2

    return op


def bravyi_kitaev_fast_generate_fermions(edge_matrix, i, j):
    """The QubitOperator for generating fermions in bravyi_kitaev_fast
    representation"""
    op = (-1j/2)*(bravyi_kitaev_fast_edge_operator_aij(edge_matrix, i, j) *
                  bravyi_kitaev_fast_edge_operator_b(edge_matrix, j) -
                  bravyi_kitaev_fast_edge_operator_b(edge_matrix, i) *
                  bravyi_kitaev_fast_edge_operator_aij(edge_matrix, i, j))
    return op
